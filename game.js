...
const SEASON=[
 {wk:1,opp:"Practice Squad",tgt:1500,reward:600},
 {wk:2,opp:"Backup Brigade",tgt:1560,reward:700},
 {wk:3,opp:"Wild Card Wolves",tgt:1620,reward:850},
 {wk:4,opp:"Division Rivals",tgt:1680,reward:1000},
 {wk:5,opp:"Playoff Hopefuls",tgt:1730,reward:1200},
 {wk:6,opp:"Conference Kings",tgt:1780,reward:1500},
 {wk:7,opp:"All-Pro Squad",tgt:1820,reward:1800},
 {wk:8,opp:"Hall of Famers",tgt:1860,reward:2200},
 {wk:9,opp:"Dream Team",tgt:1900,reward:2800},
 {wk:10,opp:"THE GOATS",tgt:1950,reward:4000}
];
const ACH=[
...
function App(){
 ...
 useEffect(()=>{try{const r=localStorage.getItem(SK);if(r){const s=JSON.parse(r);setCoins(s.coins??DS.coins);setPi(s.pi??DS.pi);setCol(s.col??[]);setLu(s.lu??{});setChal(s.chal??"std");setLastClaim(s.lastClaim??"");setStreak(s.streak??0);setSeasonWk(s.seasonWk??1);setSeasonWins(s.seasonWins??0);setAch(s.ach??[])}}catch(_){}setLoaded(true)},[]);
 const save=useCallback((o)=>{try{localStorage.setItem(SK,JSON.stringify(o))}catch(_){}}, []);
 useEffect(()=>{if(loaded)save({coins,pi,col,lu,chal,lastClaim,streak,seasonWk,seasonWins,ach})},[coins,pi,col,lu,chal,lastClaim,streak,seasonWk,seasonWins,ach,loaded,save]);
 ...
 if(!ALL||!loaded)return React.createElement("div",{style:{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",color:"#FFB800",letterSpacing:4,fontSize:16}},"LOADING…");
 ...
 const openPack=(id)=>{if((pi[id]||0)<1){sT("No packs!","#ef4444");return}setPi(p=>({...p,[id]:(p[id]||0)-1}));setOc(rollPack(id));setRev([]);setAllR(false);setApt(id);setShk(true);setScr("opening");setTimeout(()=>setShk(false),1100);unlock("first_pack")};
 ...
 const revCard=(i)=>{if(rev.includes(i))return;const n=[...rev,i];setRev(n);if(oc[i].rarity==="myth")unlock("first_legend");if(oc[i].ovr>=99)unlock("got_99");if(n.length===oc.length)setTimeout(()=>setAllR(true),600)};
 const revAll=()=>{oc.forEach((_,i)=>setTimeout(()=>{setRev(r=>[...new Set([...r,i])]);if(oc[i].rarity==="myth")unlock("first_legend");if(oc[i].ovr>=99)unlock("got_99")},i*160));setTimeout(()=>setAllR(true),oc.length*160+600)};
 ...
 const assign=(uid)=>{if(!lp)return;const c=col.find(x=>x.uid===uid);if(!c)return;if(c.ovr>CH.maxOvr){sT("Player over OVR limit ("+CH.maxOvr+")","#ef4444");return}const nlu={...lu};Object.keys(nlu).forEach(k=>{if(nlu[k]===uid)delete nlu[k]});nlu[lp.id]=uid;const used=LS.reduce((s,sl)=>{const u=nlu[sl.id];if(!u)return s;const cc=col.find(x=>x.uid===u);return cc?s+salary(cc):s},0);if(used>CH.cap){sT("Over the cap! "+fmtSal(used)+" / "+fmtSal(CH.cap),"#ef4444");return}setLu(nlu);setLp(null);setScr("lineup")};
 ...
 const acceptTrade=()=>{try{const d=JSON.parse(atob(importCode.trim()));const got=(d.give||[]).map(x=>({name:x.n,team:x.t,pos:x.p,rarity:x.r,prestige:x.pr,ovr:x.ov,value:val(x.r,x.pr),marketPrice:mp(val(x.r,x.pr)),uid:Math.random()}));setCol(c=>[...c,...got]);sT("Received "+got.length+" card(s)!","#4ade80");setImportCode("")}catch(e){sT("Invalid trade code","#ef4444")}};
 ...
 const claimDaily=()=>{const t=today();let ns=1;if(lastClaim){const d=daysBetween(lastClaim,t);if(d===1)ns=streak+1;else if(d===0){sT("Already claimed today","#ef4444");return}else ns=1}const reward=400+ns*150;setCoins(c=>c+reward);setStreak(ns);setLastClaim(t);sT("Day "+ns+" · +🪙"+fmt(reward),"#fbbf24");if(ns%5===0){setPi(p=>({...p,gold:(p.gold||0)+1}));setTimeout(()=>sT("🎁 Bonus Gold Pack!","#FFB800"),900)}if(ns>=7)unlock("streak7")};
 ...
 const maxStack=Math.max(0,...Object.values(teamCount));
 ...
 const playSeason=()=>{if(!legal){sT("Need a legal lineup first!","#ef4444");return}if(!curWeek){sT("Season complete! 🏆","#fbbf24");return}if(teamScore>=curWeek.tgt){setCoins(c=>c+curWeek.reward);setSeasonWins(w=>w+1);unlock("season_w1");if(seasonWins+1>=5)unlock("season_w5");if(seasonWk>=10)unlock("season_champ");if(seasonWk<10)setSeasonWk(seasonWk+1);sT("WIN! "+teamScore+" beat "+curWeek.tgt+" · +🪙"+fmt(curWeek.reward),"#4ade80")}else{sT("LOSS. Need "+curWeek.tgt+", had "+teamScore+". Rebuild!","#ef4444")}};
 ...
 const Btn=(p)=>React.createElement("button",p);
 const wrap=(children)=>React.createElement("div",{style:{minHeight:"100vh",background:"radial-gradient(ellipse at 15% 10%,#1a0800 0%,#05050f 50%,#000308 100%)",position:"relative",overflowX:"hidden"}},children);
 ...
 return wrap(React.createElement(React.Fragment,null,
  ...
  React.createElement("div",{style:{position:"fixed",top:0,left:0,right:0,zIndex:100,background:"rgba(3,4,10,.94)",backdropFilter:"blur(14px)",borderBottom:"1px solid #FFB80020",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 16px"}},
   React.createElement("div",{style:{display:"flex",alignItems:"center",gap:9}},
    React.createElement("div",{dangerouslySetInnerHTML:{__html:'<svg viewBox="0 0 60 50" style="height:30px;width:36px"><defs><linearGradient id="hg" x1="0" y1="0" x2="0.3" y2="1"><stop offset="0" stop-color="#FFF6D0"/><stop offset="0.4" stop-color="#F5B400"/><stop offset="0.7" stop-color="#C98A00"/><stop offset="1" stop-color="#F5C542"/></linearGradient><linearGradient id="hb" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#A0522D"/><stop offset="1" stop-color="#5c2c0c"/></linearGradient></defs><ellipse cx="30" cy="25" rx="27" ry="17" fill="url(#hb)" stroke="#3d1c08" stroke-width="1.5"/><line x1="12" y1="25" x2="48" y2="25" stroke="#FFE9C7" stroke-width="1.6" opacity="0.85"/><text x="30" y="34" text-anchor="middle" font-family="Anton,Oswald,sans-serif" font-size="26" fill="url(#hg)" stroke="#5c2c0c" stroke-width="0.5">KG</text></svg>'}}),
    React.createElement("div",{style:{fontFamily:"Anton,Oswald,sans-serif",fontSize:16,letterSpacing:2,color:"#FFB800",animation:"tG 3s ease-in-out infinite"}},"KASHTON'S GRIDIRON")),
  ...
  React.createElement("div",{style:{paddingTop:54,paddingBottom:66,minHeight:"100vh"}},
   ...
   scr==="home"&&React.createElement("div",{style:{padding:"18px 16px",maxWidth:900,margin:"0 auto"}},
    ...
    React.createElement("div",{style:{background:claimable?"linear-gradient(135deg,#78350f,#FFB80033)":"rgba(255,255,255,.03)",border:"1px solid "+(claimable?"#FFB800":"#21262d"),borderRadius:14,padding:14,marginBottom:16,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}},
     ...
     React.createElement("button",{onClick:claimDaily,disabled:!claimable,style:{background:claimable?"linear-gradient(135deg,#92400e,#fbbf24)":"#0d1117",border:"none",borderRadius:10,color:claimable?"#000":"#374151",padding:"10px 22px",fontSize:12,fontWeight:700,letterSpacing:2,cursor:claimable?"pointer":"not-allowed"}},claimable?"CLAIM 🪙"+fmt(400+(streak+1>0?(streak+ (lastClaim&&daysBetween(lastClaim,today())===1?1:1)):1)*150):"CLAIMED ✓")),
    React.createElement("div",{style:{marginBottom:14,textAlign:"center"}},React.createElement("div",{style:{fontSize:11,letterSpacing:4,color:"#374151"}},"PACK SHOP · "+ALL.length+" PLAYERS")),
   ...
   scr==="reveal"&&React.createElement("div",{style:{display:"flex",flexDirection:"column",alignItems:"center",padding:"20px 12px",gap:18}},
    ...
    React.createElement("div",{style:{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center"}},oc.map((c,i)=>React.createElement(Card,{key:c.uid||i,card:c,rev:rev.includes(i),onClick:()=>revCard(i)}))),
    allR&&React.createElement("div",{style:{animation:"su .4s ease both",textAlign:"center"}},React.createElement("button",{onClick:collect,style:{background:"linear-gradient(135deg,#92400e,#d97706,#fbbf24)",border:"none",borderRadius:10,color:"#000",padding:"13px 36px",fontSize:15,fontWeight:700,letterSpacing:3,cursor:"pointer",boxShadow:"0 0 22px #FFB80066"}},"ADD TO BINDER →")),
   ...
   scr==="binder"&&React.createElement("div",{style:{padding:16,maxWidth:1080,margin:"0 auto"}},
    React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}},
     ...
     React.createElement("button",{onClick:()=>setSellMode(s=>!s),style:{background:sellMode?"#ef444415":"transparent",border:"1px solid "+(sellMode?"#ef4444":"#21262d"),borderRadius:6,color:sellMode?"#ef4444":"#4a5568",padding:"5px 12px",fontSize:9,letterSpacing:2}},sellMode?"EXIT SELL":"SELL MODE")),
    React.createElement("div",{style:{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}},
     ...
     [["all","ALL"],["myth","LEGEND"],["legendary","LEG"],["ultra","ULTRA"],["epic","EPIC"],["rare","RARE"],["common","COM"]].map(([k,l])=>React.createElement("button",{key:k,onClick:()=>setFilterR(k),style:{background:filterR===k?(RC[k]?.border||"#FFB800")+"18":"transparent",border:"1px solid "+(filterR===k?RC[k]?.border||"#FFB800":"#21262d"),borderRadius:6,color:filterR===k?RC[k]?.border||"#FFB800":"#374151",padding:"3px 10px",fontSize:9}},l)),
     React.createElement("span",{style:{fontSize:8,letterSpacing:2,color:"#21262d",marginLeft:8}},"SORT:"),
     [["ovr","OVR"],["rarity","RAR"],["sal","SAL"],["team","TEAM"],["pos","POS"]].map(([k,l])=>React.createElement("button",{key:k,onClick:()=>setSortBy(k),style:{background:sortBy===k?"#FFB80018":"transparent",border:"1px solid "+(sortBy===k?"#FFB800":"#21262d"),borderRadius:6,color:sortBy===k?"#FFB800":"#374151",padding:"3px 10px",fontSize:9}},l))),
    ...
    React.createElement("div",{style:{display:"flex",gap:8,flexWrap:"wrap"}},fc.map(c=>React.createElement("div",{key:c.uid,style:{display:"flex",flexDirection:"column",gap:4}},
     ...
     sellMode&&!Object.values(lu).includes(c.uid)&&React.createElement("button",{onClick:()=>sellCard(c.uid),style:{width:"100%",background:"rgba(0,0,0,.5)",border:"1px solid "+RC[c.rarity]?.border+"44",borderRadius:6,color:RC[c.rarity]?.border,fontSize:9,padding:"5px 0"}},"SELL · 🪙"+fmt(c.value)),
     sellMode&&Object.values(lu).includes(c.uid)&&React.createElement("div",{style:{textAlign:"center",fontSize:8,color:"#374151",padding:"5px 0"}},"IN LINEUP"))))
   ),
   ...
   scr==="lineup"&&React.createElement("div",{style:{padding:16,maxWidth:900,margin:"0 auto"}},
    ...
    [{t:"OFFENSE",ids:["QB1","RB1","RB2","WR1","WR2","WR3","TE1","FLEX","OT1"]},{t:"DEFENSE",ids:["DE1","DE2","DT1","LB1","LB2","CB1","CB2","S1","S2"]},{t:"SPECIAL",ids:["K1"]}].map(g=>
     React.createElement("div",{key:g.t,style:{marginBottom:16}},React.createElement("div",{style:{fontSize:10,letterSpacing:3,color:"#374151",marginBottom:8}},g.t),React.createElement("div",{style:{display:"flex",gap:8,flexWrap:"wrap"}},LS.filter(s=>g.ids.includes(s.id)).map(sl=>{const u=lu[sl.id];const c=u?col.find(x=>x.uid===u):null;return React.createElement("div",{key:sl.id,style:{position:"relative"}},React.createElement(Slot,{slot:sl,card:c,onClick:slotClick}),c&&React.createElement("button",{onClick:()=>clearSlot(sl.id),style:{position:"absolute",top:-6,right:-6,width:18,height:18,borderRadius:9,background:"#ef4444",border:"none",color:"#fff",fontSize:10,cursor:"pointer"}},"×"))}))))
   ),
   ...
   scr==="pick"&&lp&&React.createElement("div",{style:{padding:16,maxWidth:900,margin:"0 auto"}},
    ...
    React.createElement("div",{style:{display:"flex",gap:8,flexWrap:"wrap"}},elig.map(c=>React.createElement("div",{key:c.uid,style:{display:"flex",flexDirection:"column",gap:4}},React.createElement(Card,{card:c,rev:true,onClick:()=>assign(c.uid)}),React.createElement("button",{onClick:()=>assign(c.uid),style:{width:"100%",background:RC[c.rarity]?.border+"18",border:"1px solid "+RC[c.rarity]?.border+"55",borderRadius:6,color:RC[c.rarity]?.border,fontSize:9,padding:"5px 0"}},"SELECT"))))
   ),
   ...
   scr==="season"&&React.createElement("div",{style:{padding:16,maxWidth:760,margin:"0 auto"}},
    ...
    React.createElement("div",{style:{fontSize:9,color:"#4a5568",marginBottom:16}},"Beat each opponent's target score with your legal lineup. Win → coins + advance. 10 weeks to the title."),
    React.createElement("div",{style:{background:"rgba(255,255,255,.03)",border:"1px solid #FFB80044",borderRadius:12,padding:16,marginBottom:14,textAlign:"center"}},
     curWeek?React.createElement(React.Fragment,null,
      ...
      React.createElement("div",{style:{fontSize:11,color:"#9CA3AF"}},"Target Score: "+React.createElement("span",{style:{color:"#ef4444",fontWeight:700}})),
      ...
      React.createElement("div",{style:{margin:"14px 0",fontSize:13}},React.createElement("span",{style:{color:"#9CA3AF"}},"Your team: "),React.createElement("span",{style:{color:teamScore>=curWeek.tgt?"#4ade80":"#ef4444",fontWeight:700,fontSize:20}},teamScore),legal?"":React.createElement("span",{style:{color:"#ef4444",fontSize:10,display:"block",marginTop:4}},"⚠ Not a legal lineup yet")),
      ...
      React.createElement("div",{style:{fontSize:18,fontWeight:700,color:"#fbbf24",margin:"8px 0"}},"SEASON COMPLETE!"),
      React.createElement("div",{style:{fontSize:11,color:"#9CA3AF"}},"You conquered all 10 weeks. Champion!"),
    ...
    React.createElement("div",{style:{fontSize:10,color:"#9CA3AF",marginBottom:8}},"Wins: "+seasonWins+" · "+React.createElement("span",null)),
   ...
   scr==="trade"&&React.createElement("div",{style:{padding:16,maxWidth:900,margin:"0 auto"}},
    ...
    React.createElement("div",{style:{fontSize:9,color:"#4a5568",marginBottom:16}},"Build an offer → generate a code → other player pastes it to receive your cards."),
    React.createElement("div",{style:{background:"rgba(255,255,255,.03)",border:"1px solid #21262d",borderRadius:12,padding:14,marginBottom:16}},
     ...
     React.createElement("div",{style:{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10,maxHeight:200,overflowY:"auto"}},col.length===0?React.createElement("div",{style:{fontSize:10,color:"#374151"}},"No cards to trade."):col.map(c=>{const on=tradeOut.includes(c.uid);return React.createElement("div",{key:c.uid,onClick:()=>setTradeOut(t=>on?t.filter(x=>x!==c.uid):[...t,c.uid]),style:{cursor:"pointer",opacity:on?1:.5,outline:on?"2px solid "+RC[c.rarity]?.border:"none",borderRadius:8}},React.createElement(Card,{card:c,rev:true,sz:"sm"}))})),
   ...
   scr==="vs"&&React.createElement("div",{style:{padding:16,maxWidth:760,margin:"0 auto"}},
    ...
    React.createElement("div",{style:{fontSize:9,color:"#4a5568",marginBottom:16}},"Compare two team codes. Copy yours below, share, paste both to see who wins."),
    ...
    [["A",cmpA,setCmpA],["B",cmpB,setCmpB]].map(([lab,v,set])=>React.createElement("div",{key:lab,style:{marginBottom:12}},React.createElement("div",{style:{fontSize:10,color:"#9CA3AF",marginBottom:4}},"TEAM "+lab+" CODE"),React.createElement("textarea",{value:v,onChange:e=>set(e.target.value),placeholder:"Paste team "+lab+" code...",style:{width:"100%",height:48,background:"#0d1117",border:"1px solid #21262d",borderRadius:6,color:"#F1F5F9",fontSize:9,padding:8,resize:"none"}}))),
    (()=>{const A=cmpA&&decodeLineup(cmpA),B=cmpB&&decodeLineup(cmpB);if(!A||!B)return React.createElement("div",{style:{fontSize:10,color:"#374151",textAlign:"center",padding:20}},"Paste both codes to compare");const win=A.score>B.score?"A":B.score>A.score?"B":"TIE";return React.createElement("div",{style:{background:"rgba(255,255,255,.03)",border:"1px solid #FFB80044",borderRadius:12,padding:16,textAlign:"center"}},
   ...
   scr==="more"&&React.createElement("div",{style:{padding:16,maxWidth:760,margin:"0 auto"}},
    ...
    React.createElement("div",{style:{background:"rgba(255,255,255,.03)",border:"1px solid #21262d",borderRadius:12,padding:16,marginBottom:16}},
     ...
     React.createElement("div",{style:{display:"flex",gap:10,marginTop:12,flexWrap:"wrap"}},Object.entries(RC).map(([r,c])=>rc[r]?React.createElement("div",{key:r,style:{fontSize:9,color:c.border}},c.label+": "+rc[r]):null)),
    ...
    React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:6}},ACH.map(a=>{const got=ach.includes(a.id);return React.createElement("div",{key:a.id,style:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",borderRadius:8,background:got?"#15803d22":"rgba(255,255,255,.02)",border:"1px solid "+(got?"#4ade8044":"#21262d"),opacity:got?1:.65}},
